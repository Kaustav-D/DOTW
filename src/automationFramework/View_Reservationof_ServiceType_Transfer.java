...
public class View_Reservationof_ServiceType_Transfer {
				...
				logger.info("Selecting Customer");
		         ...
		         try {
					...
					JavascriptExecutor js = (JavascriptExecutor)driverqa;
					 ...
					 Thread.sleep(3000);
					 js.executeScript("window.scrollBy(0,250)", "");
					 obj.Takesnap(driverqa, "D:/Test Repository/Test Screenshots/Transfer_booking_for_adults_with_children/Passenger-Details-transfer-book.jpg");
					 ...
					 Thread.sleep(4000);
					 logger.info("Viewing reservation status of service type Transfer");
		        	 ...
		        	 driverqa.findElement(NewAccoBooking.dOTWRef).sendKeys(TRNNumb);
		        	 Thread.sleep(2000);
		        	 driverqa.findElement(NewAccoBooking.CROsearch).click();
		        	// Thread.sleep(2000);
		        	 wait.until(ExpectedConditions.visibilityOfElementLocated(NewAccoBooking.finalTRN));
		        	 String result= driverqa.findElement(NewAccoBooking.finalTRN).getText();
		        	 logger.info("To validate if DOTW Ref same as TRN");
					// String expected=finalTRN.getText();
		        	
		        	 obj.Takesnap(driverqa, Config.SnapShotPath() + "/Transfer_booking_for_adults_with_children/Reservation-productType-transfers.jpg");
		        	 Assert.assertTrue(result.contains(expected));
		        	 test.log(LogStatus.INFO, "Ending Transfer Service");
					 test.log(LogStatus.PASS, "PASSED Transfer Service");
